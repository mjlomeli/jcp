...
export class LoggedInTool extends React.Component {
    constructor(props) {
        super(props);
    ...
    button(icon, tooltip, link = "#") {
        return <Link to={link} className="tool-button" style={{textDecoration: "inherit", color: "inherit"}}>
            {icon}
    ...
    favorites() {
        let icon = <svg className="tool-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
    ...
    account() {
        ...
        let components = {
            "Profile": <div className="tool-account-item">{this.props.firstName} {this.props.lastName}</div>,
            "Sign out": <div className="tool-account-item" onClick={this.onclicksignout}>Sign out</div>
        }

        return <DropdownLayout header={header} link={"#"}
                               components={components}
                               className="tool-account-drop"
                               classHeader="tool-account-header"
                               classDropper="tool-account-dropper" />
    }

    cart() {
        let icon = <svg className="tool-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        return this.button(icon, "Cart", "/cart");
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(LoggedInTool);